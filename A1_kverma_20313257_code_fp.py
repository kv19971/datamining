...
if __name__ == "__main__":
    with open("freq_items_dataset.txt", "r") as fl: #load transactions from file and store as list
        content = fl.read().split("\n")
        content = [[ci for ci in c.split(" ") if ci != ''] for c in content]
    MINSUP = 100 #minimum support
    freqitems = find_frequent_itemsets(content, MINSUP) #find frequent itemsets
    with open("resout696.txt", "w+") as fl: #write results to file
        fl.write("\n".join([",".join(r) for r in freqitems]))
